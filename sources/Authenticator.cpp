...
Authenticator::Authenticator(Susi::SusiClient &susi, BSON::Value &config)
  ...
  susi_.registerProcessor(
      ...
      [this](Susi::EventPtr event) { logout(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::users::add",
      [this](Susi::EventPtr event) { addUser(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::users::delete",
      [this](Susi::EventPtr event) { delUser(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::users::get",
      [this](Susi::EventPtr event) { getUsers(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::permissions::add",
      [this](Susi::EventPtr event) { addPermission(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::permissions::delete",
      [this](Susi::EventPtr event) { delPermission(std::move(event)); });
  susi_.registerProcessor(
      "authenticator::permissions::get",
      [this](Susi::EventPtr event) { getPermissions(std::move(event)); });
}
...
void Authenticator::login(Susi::EventPtr event) {
  ...
  if (!BCrypt::validatePassword(event->payload["password"].getString(),
                                user->pwHash)) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds{100}); // this is agains brute forcers
...
void Authenticator::addPermission(Susi::EventPtr event) {
  ...
  event->payload = BSON::Object{{"id", permission.id}};
}
...
BSON::Value Authenticator::permissionsToBSON() {
  ...
  for (auto &kv : permissionsByTopic) {
    ...
    for (auto &idAndPerm : permissions) {
      ...
      permissionObject[permission.id] =
          BSON::Object{{"pattern", permission.pattern.toAny()},
                       {"roles", BSON::Array{permission.roles.begin(),
...
bool Authenticator::BSONToPermissions(BSON::Value permissions) {
  ...
  for (auto &kv : permissions.getObject()) {
    ...
    Permission p = {.id = id, .pattern = Susi::Event{permission["pattern"]}};
...
void Authenticator::join() { susi_.join(); }
...
bool Authenticator::checkIfPayloadMatchesPattern(const BSON::Value &pattern,
...
void Authenticator::registerGuard(Permission permission) {
  susi_.registerProcessor(
      permission.pattern.topic, [permission, this](Susi::EventPtr event) {
        std::string token = getTokenFromEvent(event);
        if (checkIfPayloadMatchesPattern(permission.pattern.payload,
                                         event->payload)) {
...
void Authenticator::setupDefaults() {
  ...
  BSON::Value basePattern =
      BSON::Object{{"topic", "authenticator::(users|permissions)::.*"}};
  auto basePermission = Permission{.id = generateToken(),
...
void Authenticator::saveToFile() {
  ...
  BSON::Value saveObject =
      BSON::Object{{"users", userArray}, {"permissions", permissions}};
...
void Authenticator::loadFromFile() {
  ...
  if (!users.isArray() || users.size() == 0) {
    ...
    for (size_t i = 0; i < users.size(); i++) {
      auto &user = users[i];
      auto u =
          std::shared_ptr<User>{new User{.name = user["name"].getString(),
